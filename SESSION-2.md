...
## SMP status panel refinement
- Polls SMP status every 15 seconds.
- Re-edits the Discord status message only when the observed state changes.
- Removed verbose player-list/footer copy.
- When 0 players are online, shows: “The Server is Waiting for You, Come Fast.” under Players.
- When 1+ players are online, shows the available player names; if names are not exposed, says “Player names are unavailable.”
