  ...
  load() {
    try {
      if (fs.existsSync(this.filePath)) {
        ...
        parsed.mcStatus = parsed.mcStatus || { channelId: null, messageId: null, ip: CONFIG.smp.javaHost };
        parsed.smpConfig = parsed.smpConfig || CONFIG.smp;
        parsed.ytConfig = parsed.ytConfig || { channelId: null, ytChannelId: CONFIG.youtube.channelId, lastVideoId: null };
        parsed.ticketConfig = parsed.ticketConfig || { channelId: null, categoryChannelId: null };
        ...
        parsed.anonCount = parsed.anonCount || 0;
        return parsed;
      }
    } catch (e) {
...
// ------------------------------------------------------------------------------
// 3. PERSISTENT UNIQUE MESSAGE VIEW COUNTER (NON-EXPLOITABLE)
// ------------------------------------------------------------------------------
class MessageViewCounter {
  /**
   * Registers a unique view for a message by a user.
   * Returns { count, isNew }
   */
  static recordView(messageId, userId) {
    if (!messageId || !userId) return { count: 0, isNew: false };
    const views = db.get('views', {});
    if (!views[messageId]) {
      views[messageId] = { count: 0, viewers: [] };
    }
    const entry = views[messageId];
    if (!entry.viewers.includes(userId)) {
      entry.viewers.push(userId);
      entry.count = entry.viewers.length;
      db.set('views', views);
      return { count: entry.count, isNew: true };
    }
    return { count: entry.count, isNew: false };
  }

  static getViews(messageId) {
    if (!messageId) return 0;
    const views = db.get('views', {});
    return views[messageId]?.count || 0;
  }

  /**
   * Builds the clean, minimal secondary View Counter button
   */
  static buildViewButton(messageId, explicitCount = null) {
    const count = explicitCount !== null ? explicitCount : MessageViewCounter.getViews(messageId);
    return new ButtonBuilder()
      .setCustomId('btn_msg_views')
      .setLabel(`${count} Views`)
      .setEmoji('👁️')
...
// ------------------------------------------------------------------------------
// 4. MINECRAFT SMP TELEMETRY (100% LIVE SYNC VIA API.MCSTATUS.IO)
// ------------------------------------------------------------------------------
let smpCache = null;
let smpCacheTime = 0;
...
function buildNeonStatusEmbed(data, views = 0) {
  ...
  const embed = new EmbedBuilder()
    ...
    .setFooter({ text: `⚡ NETHRION 2.0 • 👁️ ${views} Views • Mindzard Community` })
    .setTimestamp();
...
function buildNeonIpEmbed(data, views = 0) {
  return new EmbedBuilder()
    ...
    .setFooter({ text: `⚡ Copy the IP above to connect • 👁️ ${views} Views • Mindzard Community` })
    .setTimestamp();
}

function buildNeonPanelEmbed(data, views = 0) {
  const isOnline = data.isOnline;
  const color = isOnline ? '#00FF9D' : '#FF0055';
  ...
  return new EmbedBuilder()
    ...
    .setFooter({ text: `⚡ Auto-refreshes every 60s • 👁️ ${views} Views • NETHRION Telemetry` })
    .setTimestamp();
}

function buildNeonPanelButtons(messageId = null) {
  const views = messageId ? MessageViewCounter.getViews(messageId) : 0;
  return new ActionRowBuilder().addComponents(
    ...
    MessageViewCounter.buildViewButton(messageId, views)
  );
}

function buildNeonTicketPanelEmbed(views = 0) {
  return new EmbedBuilder()
    ...
    .setFooter({ text: `Mindzard Community Support • 👁️ ${views} Views • Click below to start` })
    .setTimestamp();
}

function buildAnonPanelEmbed(views = 0) {
  return new EmbedBuilder()
    ...
    .setFooter({ text: `Mindzard Community • 👁️ ${views} Views • Confidential & Safe` })
    .setTimestamp();
}

// ------------------------------------------------------------------------------
// 6. AI ENGINE (GEMINI 2.0 PRIMARY + GROQ RESILIENT FALLBACK)
// ------------------------------------------------------------------------------
...
// ------------------------------------------------------------------------------
// 7. ROLE HELPER (GET OR CREATE WITH PERMISSION CHECKS)
// ------------------------------------------------------------------------------
async function getOrCreateRole(guild, roleName, color = '#3498db') {
  ...
  if (!role) {
    try {
      role = await guild.roles.create({
        name: roleName,
        color,
        reason: 'Auto-created by NETHRION Bot'
      });
    } catch (e) {
      console.error('[Role] Failed to create role:', e.message);
    }
  }
  return role;
}
...
// ------------------------------------------------------------------------------
// 8. DISCORD CLIENT INITIALIZATION
// ------------------------------------------------------------------------------
...
// ------------------------------------------------------------------------------
// 9. YOUTUBE UPLOAD POLLER (MINDZARD COMMUNITY NOTIFICATIONS)
// ------------------------------------------------------------------------------
async function checkYouTubeUploads() {
  const ytConfig = db.get('ytConfig');
  if (!ytConfig || !ytConfig.channelId || !ytConfig.ytChannelId) return;

  const url = `https://www.youtube.com/feeds/videos.xml?channel_id=${ytConfig.ytChannelId}`;
  https.get(url, (res) => {
    ...
    res.on('end', async () => {
      try {
        ...
        const authorMatch = data.match(/<name>([^<]+)<\/name>/);
        if (!idMatch) return;

        const videoId = idMatch[1];
        if (ytConfig.lastVideoId !== videoId) {
          ytConfig.lastVideoId = videoId;
          db.set('ytConfig', ytConfig);

          const channel = await client.channels.fetch(ytConfig.channelId).catch(() => null);
          if (channel) {
            const videoUrl = `https://www.youtube.com/watch?v=${videoId}`;
            const embed = new EmbedBuilder()
              .setTitle(`🎬 New Upload: ${titleMatch ? titleMatch[1] : 'New Video'}`)
              .setURL(videoUrl)
              .setColor('#FF0000')
              .setDescription(`**${authorMatch ? authorMatch[1] : 'Mindzard'}** just uploaded a new video!\n\n[Watch on YouTube](${videoUrl})`)
              .setImage(`https://i.ytimg.com/vi/${videoId}/maxresdefault.jpg`)
              .setFooter({ text: 'Mindzard Official YouTube Alerts' })
              .setTimestamp();

            await channel.send({
              content: `📢 **@everyone** Mindzard uploaded a new video! ${videoUrl}`,
              embeds: [embed]
            }).catch(() => {});
          }
        }
...
// ------------------------------------------------------------------------------
// 10. MESSAGE EVENT & COMMAND DISPATCHER
// ------------------------------------------------------------------------------
client.on('messageCreate', async (message) => {
  ...
  // Robust permission detection (Owner always has full bypass)
    ...
    const adminHelp = new EmbedBuilder()
      ...
      .addFields(
        ...
        },
        {
          ...
          value: [
            '`sp smp-set <java-ip[:port]> [bedrock-ip] [bedrock-port]` — Fully adjustable SMP endpoints (instantly synced).',
            '`sp smp-panel` — Deploy the live auto-updating SMP status panel with View Counter.',
        ...
        },
        {
          ...
          value: [
            ...
            '`sp backup` / `sp backups` — Create or list structural ZIP backups.',
            '`sp views [message_id]` — Inspect unique view counts on messages.',
    ...
    const helpEmbed = new EmbedBuilder()
      ...
      .addFields({
        ...
        value: [
          ...
          '`sp views` — View analytics on bot messages.',
  ...
  // --------------------------------------------------------------------------
  // SP SMP / SP IP / SP SMP-PANEL / SP SMP-SET (FULLY ADJUSTABLE DYNAMIC SYNC)
  // --------------------------------------------------------------------------
  if (subCmd === 'smp') {
    message.channel.sendTyping().catch(() => {});
    const status = await getLiveSMPStatus(true);
    const views = MessageViewCounter.getViews(message.id);
    const embed = buildNeonStatusEmbed(status, views);
    const sent = await message.channel.send({
      embeds: [embed],
      components: [new ActionRowBuilder().addComponents(MessageViewCounter.buildViewButton(null, 1))]
    });
    // Register initial author view
    MessageViewCounter.recordView(sent.id, message.author.id);
    return;
  }
  ...
  // DYNAMIC & ADJUSTABLE SMP-SET:
  // Replaces Java/Bedrock endpoints, handles typos (like : instead of .), updates mcStatus,
  // invalidates cache, pings live status, and instantly updates the live SMP panel!
  if (subCmd === 'smp-set') {
    if (!isAdmin) return message.reply('❌ This command is restricted to Admins!');
    let javaRaw = args[1];
    if (!javaRaw) {
      return message.reply('❌ Usage: `sp smp-set <java-ip[:port]> [bedrock-ip] [bedrock-port]`\nExample: `sp smp-set nethrionsmp.pixelforge.gg 15.235.165.81 26091`');
    }

    // Smart typo auto-correction: if user typed "nethrionsmp:pixelforge.gg" (colon instead of dot), fix it
    if (javaRaw.includes(':')) {
      const colonParts = javaRaw.split(':');
      if (!/^\d+$/.test(colonParts[1])) {
        // Not a port number, so user typed a colon instead of a dot in the domain!
        javaRaw = colonParts.join('.');
      }
    }

    let javaHost = javaRaw;
    let javaPort = 25565;

    if (javaRaw.includes(':')) {
      const p = javaRaw.split(':');
      javaHost = p[0];
      javaPort = parseInt(p[1], 10) || 25565;
    }
    ...
    // 2. Update mcStatus.ip
    const mcStatus = db.get('mcStatus', {});
    mcStatus.ip = javaHost;
    db.set('mcStatus', mcStatus);

    // 3. Invalidate cache and fetch fresh live telemetry
    smpCache = null;
    message.channel.sendTyping().catch(() => {});
    const freshStatus = await getLiveSMPStatus(true);

    // 4. Instantly update live SMP panel if it exists
    let panelUpdated = false;
    if (mcStatus.channelId && mcStatus.messageId) {
      try {
        const panelChan = await client.channels.fetch(mcStatus.channelId).catch(() => null);
        if (panelChan) {
          const panelMsg = await panelChan.messages.fetch(mcStatus.messageId).catch(() => null);
          if (panelMsg) {
            const views = MessageViewCounter.getViews(panelMsg.id);
            const panelEmbed = buildNeonPanelEmbed(freshStatus, views);
            const panelBtns = buildNeonPanelButtons(panelMsg.id);
            await panelMsg.edit({ embeds: [panelEmbed], components: [panelBtns] }).catch(() => {});
            panelUpdated = true;
          }
        }
      } catch (e) {}
    ...
    const confirmEmbed = new EmbedBuilder()
      .setTitle('✅ ＳＭＰ  ＥＮＤＰＯＩＮＴＳ  ＵＰＤＡＴＥＤ')
      .setColor('#00FF9D')
      .setDescription(
        '**Minecraft SMP settings have been dynamically replaced and verified!**\n━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n' +
        `🌐 **Java Server:** \`${javaHost}:${javaPort}\`\n` +
        `🪨 **Bedrock Server:** \`${bedrockHost}:${bedrockPort}\`\n` +
        `📡 **Live Telemetry:** ${freshStatus.isOnline ? '🟢 Online (' + freshStatus.playersOnline + ')' : '🔴 Offline'}\n` +
        `🔄 **Live Panel Sync:** ${panelUpdated ? '✅ Instant panel message refreshed' : '⚠️ No active panel message found'}`
      )
      .setFooter({ text: 'All commands and auto-pollers now use this new endpoint.' })
      .setTimestamp();
  ...
  if (cmdLower === 'ticket-panel') {
    if (!isAdmin) return message.reply('❌ This command is restricted to Admins!');
    const embed = buildNeonTicketPanelEmbed(1);
    const row = new ActionRowBuilder().addComponents(
      ...
      MessageViewCounter.buildViewButton(null, 1)
    );
  ...
  if (cmdLower === 'anon-panel' || cmdLower === 'anon') {
    if (!isAdmin) return message.reply('❌ This command is restricted to Admins!');
    const embed = buildAnonPanelEmbed(1);
    const row = new ActionRowBuilder().addComponents(
      new ButtonBuilder()
  ...
  if (cmdLower === 'roles-panel') {
    ...
    const embed = new EmbedBuilder()
      ...
      .setColor('#8B5CF6')
      .setDescription('Choose a channel from the dropdown below to **toggle** its notification role. Select again anytime to undo.')
      .setTimestamp();
  ...
  // --------------------------------------------------------------------------
  // SP VIEWS (INSPECT UNIQUE VIEWS)
  // --------------------------------------------------------------------------
  if (subCmd === 'views') {
    const targetMsgId = args[1] || (message.reference ? message.reference.messageId : null);
    if (!targetMsgId) {
      return message.reply('❌ Usage: Reply to a message or provide ID: `sp views <message_id>`');
    }
    const views = MessageViewCounter.getViews(targetMsgId);
    const embed = new EmbedBuilder()
      .setTitle('👁️ Message View Analytics')
      .setColor('#00F0FF')
      .setDescription(`Target Message ID: \`${targetMsgId}\`\nUnique Registered Views: **${views}**`)
      .setFooter({ text: 'Non-overlapping, persistent view counter' })
      .setTimestamp();
    return message.reply({ embeds: [embed] });
  }

  // --------------------------------------------------------------------------
  // SP ASK / SP SUGGEST / SP REPORT / SP TASK / SP DIAGNOSE / SP BACKUP
  // --------------------------------------------------------------------------
  if (subCmd === 'ask') {
...
// ------------------------------------------------------------------------------
// 11. INTERACTIVE BUTTON, MODAL, & MENU HANDLER
// ------------------------------------------------------------------------------
client.on('interactionCreate', async (interaction) => {
  // 1. Button interactions
  if (interaction.isButton()) {
    const { customId, guild, member, channel, message } = interaction;

    // Record view on the message when user interacts
    if (message?.id && interaction.user?.id) {
      MessageViewCounter.recordView(message.id, interaction.user.id);
    }

    if (customId === 'btn_msg_views') {
      const res = MessageViewCounter.recordView(message.id, interaction.user.id);
      // Update the button counter dynamically on the message if possible
      try {
        const rows = message.components.map(row => {
          const newRow = ActionRowBuilder.from(row);
          newRow.components = newRow.components.map(comp => {
            if (comp.data.custom_id === 'btn_msg_views') {
              return MessageViewCounter.buildViewButton(message.id, res.count);
            }
            return comp;
          });
          return newRow;
      ...
      if (res.isNew) {
        return interaction.reply({
          content: `👁️ **Unique View Registered!** You are viewer **#${res.count}** for this message.`,
          ephemeral: true
        });
      } else {
        return interaction.reply({
          content: `👁️ You have already viewed this message. Total unique views: **${res.count}**.`,
          ephemeral: true
        });
      }
    } else if (customId === 'btn_refresh_smp') {
      await interaction.deferUpdate();
      const status = await getLiveSMPStatus(true);
      const views = MessageViewCounter.getViews(message.id);
      const embed = buildNeonPanelEmbed(status, views);
      const buttons = buildNeonPanelButtons(message.id);
  ...
  // 2. Modal Submission (Anonymous messages)
  ...
  // 3. String Select Menu (Dynamic Channel Notification Roles)
  if (interaction.isStringSelectMenu()) {
    if (interaction.customId === 'dynamic_role_select') {
      ...
      if (!targetChan) {
        return interaction.editReply({ content: '❌ Selected channel no longer exists!' });
      }

      const roleName = `🔔 #${targetChan.name}`;
      const role = await getOrCreateRole(interaction.guild, roleName);
      if (!role) {
        return interaction.editReply({ content: '❌ Could not manage role due to bot permissions hierarchy.' });
      }
...
async function createTicketChannel(guild, member, topic) {
  ...
  const ticketMsg = await chan.send({ content: member.toString(), embeds: [embed], components: [row] });
  MessageViewCounter.recordView(ticketMsg.id, member.id);
  return chan;
}

// ------------------------------------------------------------------------------
// 12. CLIENT BOOTSTRAP & BACKGROUND TASKS
// ------------------------------------------------------------------------------
client.once('ready', () => {
  console.log('====================================================');
  ...
  // 1. Auto-Updating SMP Live Panel (every 60 seconds)
  setInterval(async () => {
    try {
      ...
      const status = await getLiveSMPStatus(true);
      const views = MessageViewCounter.getViews(msg.id);
      const embed = buildNeonPanelEmbed(status, views);
      const buttons = buildNeonPanelButtons(msg.id);
      await msg.edit({ embeds: [embed], components: [buttons] }).catch(() => {});
    } catch (e) {}
  }, 60000);

  // 2. YouTube poller (every 5 minutes)
  checkYouTubeUploads().catch(() => {});
  setInterval(checkYouTubeUploads, CONFIG.youtube.pollIntervalMs);
});
