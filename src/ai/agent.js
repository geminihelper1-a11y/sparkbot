...
const {formatToolResult,normalizeAiReply}=require('../ui/formatters');
...
class Agent{
  ...
  async run({message,guild,member,text}){
    const clean=redact(text);const isGreeting=/^(?:hi|hello|hey|hey there|yo|good morning|good afternoon|good evening)[!.?\s]*$/i.test(clean);this.repos.addTurn(guild.id,member.id,message.channel.id,'user',clean);
    ...
    try{
      ...
      if(!functionSteps.length){const publicReply=normalizeAiReply(response.text,{allowGreeting:isGreeting});this.repos.addTurn(guild.id,member.id,message.channel.id,'assistant',publicReply);return {state:'SUCCESS',reply:publicReply,provider:response.provider};}
      ...
      for(const step of functionSteps.slice(0,4)){
        ...
        finalText=formatToolResult(step.name,result);
        if(response.interactionId && this.router.gemini?.continueWithToolResult){const cont=await this.router.gemini.continueWithToolResult({previousInteractionId:response.interactionId,toolName:step.name,callId:step.id,result,tools:toolDefs});finalText=normalizeAiReply(cont.text||finalText,{allowGreeting:false});}
      }
      finalText=normalizeAiReply(finalText,{allowGreeting:isGreeting});this.repos.addTurn(guild.id,member.id,message.channel.id,'assistant',finalText);return {state:'SUCCESS',reply:finalText,provider:response.provider,actionPreview};
